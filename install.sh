...
function createJumpboxVM() {
  ...
  if returnsSomething govc vm.info "${vm_name}" ; then
    echo >&2 "Jumpbox VM already exists, skipping creation"
  else
...
function copySSHKeysToJumpbox() {
  echo 'Copying SSH keys to jumpbox'
...
function main() {
  ...
  addHostToSSHConfig 'jumpbox' "$jumpbox_ip" 'ubuntu'
  copySSHKeysToJumpbox

  echo
  echo "SSH into ${vm_name}:"
  echo " ssh -F .ssh/config jumpbox"
  echo
}
